...
export function ExploreGrapesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-3xl text-foreground">Grapes</h1>
      ...
      <div className="flex flex-wrap gap-2">
        <span className="text-sm text-muted-foreground self-center">Color:</span>
        {["", "red", "white"].map((c) => (
          <Button
            key={c || "all"}
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((g) => (
            <Link key={g.id} to={`/explore/grapes/${g.id}`} className="no-underline">
              <Panel variant="oak" className="h-full transition-colors duration-200 hover:border-accent/50">
                <h3 className="font-serif text-xl text-foreground">{g.displayName}</h3>
