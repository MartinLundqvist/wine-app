...
CREATE TABLE IF NOT EXISTS "wine_style" (
	...
	"notes" text,
	CONSTRAINT "wine_style_climate_check" CHECK ((climate_min IS NULL AND climate_max IS NULL) OR (climate_min IS NOT NULL AND climate_max IS NOT NULL AND climate_min >= 1 AND climate_min <= 5 AND climate_max >= 1 AND climate_max <= 5 AND climate_min <= climate_max))
);
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "wine_style_grape" (
	...
	CONSTRAINT "wine_style_grape_percentage_check" CHECK (percentage IS NULL OR (percentage >= 0 AND percentage <= 100))
);
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "wine_style_structure" (
	...
	CONSTRAINT "wine_style_structure_values_check" CHECK (min_value >= 1 AND min_value <= 5 AND max_value >= 1 AND max_value <= 5 AND min_value <= max_value)
);
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "wine_style_aroma_cluster" (
	...
	CONSTRAINT "wine_style_aroma_cluster_intensity_check" CHECK (intensity_min >= 1 AND intensity_min <= 5 AND intensity_max >= 1 AND intensity_max <= 5 AND intensity_min <= intensity_max)
);
--> statement-breakpoint
